...
class Paper extends Component {
    ...
    deletePaper(id,filename) {
        if (id !== null) {
            ...
            console.log(body)
            fetch(url + '/api/deletePaper', { method: "DELETE", headers: headers,body:JSON.stringify(body) })
                .then(res => res.json())
                .then(data => {
            ...
            alert('Paper must not be null')
        }
    }
    createPaper() {
        const { title,exam,fileData} = this.state
        if (title.length === 0 && fileData!==null) {
            ...
            fetch(url + '/api/addPastPaper', { method: "POST", body: data })
                .then(res => res.json())
                .then(data => {
                    if (data.message === 'Success') {
            ...
            <div>
                ...
                {this.state.renderPage &&
                    <div>
                                            ...
                                            <div className="card-header">
                                                <div className='row'>
                                                    <div className='col-md-4'>
                                                        <b style={{ fontSize: 20 }}>Past Papers</b>
                                                    </div>
                                                    <div className='col-md-6'>
                                                    </div>
                                                    <div className='col-md-2'>
                                                        <button onClick={() => this.setState({ showPaperModal: true })} className='btn btn-primary'>Add Paper</button>
                                                    </div>
                                                </div>
                                            </div>
                                                        ...
                                                        <div><table className="table table-hover">
                                                            ...
                                                            {this.state.customer && <tbody >
                                                                ...
                                                                {this.state.customer.length > 0 && this.state.customer.map((user, index) => {
                                                                    let link = url+'/api/getPastPaper'+ user.fileLink
                                                                    let examNames = ["FCPS","FCPS1","FCPS2"]
                                                                    return (
                                                                        <tr key={index}>
                                                                            <td>{count + index + 1}</td>
                                                                            <td>{user.title}</td>
                                                                            <td>{examNames[user.exam]}</td>
                                                                            <td><a href={link} target='_blank'>{user.title}</a></td>
                                                                            <td><button onClick={() => this.deletePaper(user._id,user.fileLink)} className='btn btn-danger'>Delete</button></td>
                                                                        </tr>
                                                                    )
                                                                }
                                                                )}
                        ...
                        <Modal centered toggle={() => this.setState({ showPaperModal: false })} isOpen={this.state.showPaperModal} onClose={this.onCloseModal}>
                            <br />
                            <br />
                            <h2 style={{ textAlign: "center" }}>Add Paper</h2>
                            <br />
                            <input
                                type='text'
                            ...
                            <br />
                            <p>Exam:</p>
                            <select placeholder='Exam' onChange={e=>{
                            ...
                            <div className='row'>
                                ...
                                <div className='col-md-3'>
                                    <button className='btn btn-primary' onClick={this.createPaper} style={{ width: 100 }}>Create</button>
                                </div>
                                <div className='col-md-2'>
...
export default connect(mapStateToProps, mapActionsToProps)(Paper)